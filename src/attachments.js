...
export default class extends Controller {
  ...
  handleFileClick() {
    if (this.hasMaxFilesValue && this.activeFiles.length >= this.maxFilesValue) {
  ...
  async handleNewFiles(event) {
    ...
    if (this.hasMaxFilesValue && files.length + this.activeFiles.length > this.maxFilesValue) {
      setMessage('alert', `Maximum files allowed: ${this.maxValue}`)
      return false
    }

    const approvedFiles = await Promise.all(files.map(file => this.approveFile(file)))
  ...
  handleFileProgress = event => {
    ...
    switch (event.state) {
      case 'uploading':
        attachment.querySelector('.progress-bar').style.width = `${event.progress}%`
        break
      case 'error':
        attachment.remove()
